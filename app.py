...
def register():
    ...
    if request.method == "POST":
        ...
        if not errors:
            try:
                users.create_user(username, generate_password_hash(password1))
...
def create_review(movie_id):
    ...
    fetched_movie = movies.get_movie_by_id(movie_id)
    ...
    return render_template(
        "movies/create_review.html", movie=fetched_movie, movie_genres=movie_genres, body=""
    )
